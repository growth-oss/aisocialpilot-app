...
- Server: `server/index.js` (Express, ~2700 lines)
...
- **Precision Content Engine** (Content tab):
  ...
  - Approve → queue → auto-post during regular run (PHASE D in prompt.js)
  - Queue-based posting with human-like timing (60/40 browse-first split, 15-40min gaps)
...
9. **Brief status flow**: `pending` → `queued` (Approve) → `posted` (Claude during run)
...
| `data/clients/{id}/config.json` | Client config incl. schedule + smartSchedule |
| `data/clients/{id}/leadgen/precision-briefs.json` | Content briefs |
| `data/clients/{id}/assets/precision/` | Generated images |
